...
export const POST: RequestHandler = async ({ params, request, fetch }) => {
    try {
        ...
        const apiUrl = `${BASE_API_URL}/audits/engagements/${params.id}/close/`;
        ...
        if (!response.ok) {
            ...
            return json({ error: errorData.detail || errorData.error || 'Failed to close engagement' }, { status: response.status });
        }
