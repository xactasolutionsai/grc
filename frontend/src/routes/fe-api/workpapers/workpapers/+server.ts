...
export const GET: RequestHandler = async ({ fetch, url, request }) => {
	try {
		...
		const apiUrl = `${BASE_API_URL}/workpapers/workpapers/${searchParams ? '?' + searchParams : ''}`;
		...
		if (!response.ok) {
			error(response.status, 'Failed to fetch workpapers');
		}
...
export const POST: RequestHandler = async ({ fetch, request }) => {
	try {
		...
		const apiUrl = `${BASE_API_URL}/workpapers/workpapers/`;
		...
		if (!response.ok) {
			error(response.status, 'Failed to create workpaper');
		}
