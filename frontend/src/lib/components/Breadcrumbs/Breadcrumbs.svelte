...
<ol class="flex items-center gap-4 h-6 overflow-hidden whitespace-nowrap">
    {#each $breadcrumbs as c, i}
        {#if i == $breadcrumbs.length - 1}
			<span
                    class="max-w-[64ch] overflow-hidden text-sm text-white/70 font-semibold antialiased"
        ...
        {:else}
            <li>
                {#if c.href}
                    <a
                            class="max-w-[64ch] overflow-hidden unstyled text-sm text-white/90 hover:text-tertiary-300 font-semibold antialiased whitespace-nowrap transition-colors"
                            data-testid="crumb-item"
                            href={c.href}
