...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Successfully updated timeline for {updated_count} engagements!')
        )
    ...
    def update_engagement_timeline(self, engagement):
        ...
        if engagement.status in ['in_progress', 'fieldwork', 'review', 'submitted', 'closed']:
            if not engagement.actual_start_date:
                # Start 1-5 days after creation
                days_after_creation = random.randint(1, 5)
                engagement.actual_start_date = created_date.date() + timedelta(days=days_after_creation)
        ...
        if engagement.status in ['submitted', 'closed']:
            if not engagement.results_submitted_at:
                ...
                if engagement.actual_start_date:
                    ...
                    submit_date = engagement.actual_start_date + timedelta(days=days_after_start)
                    ...
                    if not engagement.results_submitted_by:
                        engagement.results_submitted_by = engagement.assigned_auditor or engagement.created_by
        ...
        if engagement.status == 'closed':
            if not engagement.closed_at:
                ...
                if engagement.results_submitted_at:
                    ...
                    close_date = engagement.results_submitted_at.date() + timedelta(days=days_after_submit)
                    ...
                    if not engagement.closed_by:
                        engagement.closed_by = engagement.engagement_lead or engagement.assigned_auditor or engagement.created_by
        ...
        if engagement.status in ['submitted', 'closed'] and not engagement.findings_summary:
            findings_templates = [
        ...
        if engagement.status in ['submitted', 'closed'] and not engagement.recommendations:
