...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not audit_plans:
            self.stdout.write(self.style.ERROR('No audit plans found. Please create audit plans first.'))
            return
            
        if not audit_entities:
            self.stdout.write(self.style.ERROR('No audit entities found. Please create audit entities first.'))
            return
    ...
    def get_or_create_demo_users(self):
        ...
        demo_users = [
            {'email': 'auditor1@company.com', 'first_name': 'Alice', 'last_name': 'Auditor'},
            {'email': 'auditor2@company.com', 'first_name': 'Bob', 'last_name': 'Smith'},
            {'email': 'auditor3@company.com', 'first_name': 'Carol', 'last_name': 'Johnson'},
            {'email': 'audit.manager@company.com', 'first_name': 'David', 'last_name': 'Manager'},
            {'email': 'senior.auditor@company.com', 'first_name': 'Eva', 'last_name': 'Wilson'},
        ]
    ...
    def get_audit_entities(self):
        ...
        return list(AuditEntity.objects.filter(is_active=True)[:10])  # Get up to 10 active entities
    ...
    def create_demo_engagements(self, users, audit_plans, audit_entities):
        ...
        engagement_templates = [
            ...
            },
            {
                ...
                'actual_cost': 8667,
                'tags': ['Security', 'Incident-Response', 'Cybersecurity', 'Post-Incident']
            },
            {
        ...
        for i, template in enumerate(engagement_templates):
            ...
            if template['status'] in ['in_progress', 'fieldwork', 'review', 'submitted', 'closed']:
                actual_start = planned_start + timedelta(days=random.randint(0, 5))
            
            if template['status'] in ['submitted', 'closed']:
                results_submitted_at = timezone.make_aware(
                    (actual_start + timedelta(days=random.randint(10, 30))).replace(hour=14, minute=0, second=0, microsecond=0)
            ...
            if template['status'] == 'closed':
                closed_at = timezone.make_aware(
                    (results_submitted_at.date() + timedelta(days=random.randint(1, 7))).replace(hour=16, minute=0, second=0, microsecond=0)
                ) if results_submitted_at else None
            ...
            self.stdout.write(f'Created engagement: {engagement.title} (Status: {engagement.status})')
        ...
        for template in overdue_engagements:
            ...
            self.stdout.write(f'Created overdue engagement: {engagement.title} (Status: {engagement.status})')
        ...
        self.stdout.write(f'Draft: {AuditEngagement.objects.filter(status="draft").count()}')
        self.stdout.write(f'In Progress: {AuditEngagement.objects.filter(status="in_progress").count()}')
        self.stdout.write(f'Fieldwork: {AuditEngagement.objects.filter(status="fieldwork").count()}')
        self.stdout.write(f'Review: {AuditEngagement.objects.filter(status="review").count()}')
        self.stdout.write(f'Submitted: {AuditEngagement.objects.filter(status="submitted").count()}')
        self.stdout.write(f'Closed: {AuditEngagement.objects.filter(status="closed").count()}')
        self.stdout.write(f'Overdue engagements: {AuditEngagement.objects.filter(planned_end_date__lt=date.today(), status__in=["draft", "in_progress", "fieldwork", "review"]).count()}')
