...
from audits.models import (
    Checklist, ChecklistItem, AuditEngagement, 
    ChecklistExecution, ChecklistItemResult
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            self.stdout.write(self.style.SUCCESS(f'  Added {len(items)} items to SOC 2 checklist'))
        ...
        if created:
            ...
            self.stdout.write(self.style.SUCCESS(f'  Added {len(items)} items to ISO 27001 checklist'))
        ...
        if created:
            ...
            self.stdout.write(self.style.SUCCESS(f'  Added {len(items)} items to GDPR checklist'))
        ...
        if options['with_execution']:
            ...
            if engagement:
                ...
                if created:
                    ...
                    execution.completed_items = execution.item_results.exclude(result='not_tested').count()
                    execution.save()
                    
                    self.stdout.write(self.style.SUCCESS(f'Created sample execution: {execution}'))
            else:
                self.stdout.write(self.style.WARNING('No audit engagements found. Skipping execution creation.'))
