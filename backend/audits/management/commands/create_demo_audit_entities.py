...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Successfully created demo audit entities with hierarchy!"
            )
        )
    ...
    def create_demo_entities(self, users):
        ...
        for process_data in processes:
            ...
            self.stdout.write(
                f"Created process: {entity.name} (Parent: {entity.parent.name})"
            )
        ...
        for system_data in systems:
            ...
            self.stdout.write(
                f"Created system: {entity.name} (Parent: {entity.parent.name})"
            )
        ...
        for vendor_data in vendors:
            ...
            self.stdout.write(
                f"Created vendor: {entity.name} (Parent: {entity.parent.name})"
            )
        ...
        for compliance_data in compliance_domains:
            ...
            self.stdout.write(
                f"Created compliance domain: {entity.name} (Parent: {entity.parent.name})"
            )
        ...
        for inactive_data in inactive_entities:
            ...
            self.stdout.write(
                f"Created inactive entity: {entity.name} (Parent: {entity.parent.name})"
            )
        ...
        self.stdout.write(
            f"Business Units: {AuditEntity.objects.filter(entity_type='business_unit').count()}"
        )
        self.stdout.write(
            f"Processes: {AuditEntity.objects.filter(entity_type='process').count()}"
        )
        self.stdout.write(
            f"Systems: {AuditEntity.objects.filter(entity_type='system').count()}"
        )
        self.stdout.write(
            f"Vendors: {AuditEntity.objects.filter(entity_type='vendor').count()}"
        )
        self.stdout.write(
            f"Compliance Domains: {AuditEntity.objects.filter(entity_type='compliance_domain').count()}"
        )
        self.stdout.write(
            f"Active entities: {AuditEntity.objects.filter(is_active=True).count()}"
        )
        self.stdout.write(
            f"Inactive entities: {AuditEntity.objects.filter(is_active=False).count()}"
        )
