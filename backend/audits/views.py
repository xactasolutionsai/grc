...
from .serializers import (
    AuditEntitySerializer,
    AuditPlanSerializer,
    RelatedObjectsSerializer,
    AuditPlanApprovalSerializer,
    AuditPlanApprovalActionSerializer,
    AuditEngagementSerializer,
    AuditEngagementActionSerializer,
    EntityProcessActivitySerializer,
    ChecklistSerializer,
...
class AuditEntityViewSet(viewsets.ModelViewSet):
    ...
    queryset = AuditEntity.objects.select_related(
        "parent", "owner", "team_member"
    ).all()
    ...
    def processes(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            if (
                identifier
                and entity.process_activities.filter(identifier=identifier).exists()
            ):
                return Response(
                    {"detail": "identifier must be unique per entity"},
    ...
    @action(
        ...
        url_path=r"processes/(?P<process_id>[^/.]+)",
    )
    def process_detail(self, request, pk=None, process_id=None):
        ...
        try:
            ...
            return Response(
                {"detail": "Process not found"}, status=status.HTTP_404_NOT_FOUND
            )
        if request.method.lower() == "delete":
            process.delete()
            return Response(status=status.HTTP_204_NO_CONTENT)
        # update
        serializer = EntityProcessActivitySerializer(
        ...
        if serializer.is_valid():
            ...
            if (
                identifier
                and entity.process_activities.exclude(pk=process.pk)
    ...
    def related(self, request, pk=None):
        ...
        try:
            ...
            compliance_audits = ComplianceAssessment.objects.select_related(
                "framework"
            ).all()[:5]
    ...
    def upload_org_structure(self, request, pk=None):
        ...
        entity.save()
        return Response(
            {"org_structure_url": entity.org_structure.url}, status=status.HTTP_200_OK
        )
    ...
    def get_hierarchy(self, request):
        ...
        entities = (
            AuditEntity.objects.select_related("parent", "owner", "team_member")
        ...
        if request.user and request.user.is_authenticated:
            ...
            if my_entities and my_entities.lower() == "true":
                entities = entities.filter(
                    Q(owner=request.user) | Q(team_member=request.user)
                )
            ...
            for row_num, row in enumerate(csv_reader, start=2):  # Start at 2 for header
                try:
                    ...
                    if entity_type not in valid_types:
                        errors.append(
                            f"Row {row_num}: Invalid type '{entity_type}'. Must be one of: {', '.join(valid_types)}"
                        )
                        continue
                    ...
                    if parent_name:
                        try:
                            ...
                            errors.append(
                                f"Row {row_num}: Parent '{parent_name}' not found"
                            )
                            continue
    ...
    def processes_lookup(self, request):
        ...
        processes = (
            EntityProcessActivity.objects.select_related("audit_entity")
            .all()
            .order_by("audit_entity__name", "name")
        )
        data = []
            ...
            data.append(
                {
                    ...
                    "display_name": f"{process.identifier} – {process.name} ({process.audit_entity.name})"
                    if process.identifier
                    else f"{process.name} ({process.audit_entity.name})",
                }
            )
...
class AuditPlanViewSet(viewsets.ModelViewSet):
    ...
    def add_approver(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": "Approver not found"}, status=status.HTTP_400_BAD_REQUEST
            )

        approval, created = AuditPlanApproval.objects.get_or_create(
            audit_plan=audit_plan, approver=approver, defaults={"status": "pending"}
        )
...
class AuditEngagementViewSet(viewsets.ModelViewSet):
    ...
    search_fields = [
        ...
        "audit_plan__title",
        "entity__name",
        "assigned_auditor__email",
        "engagement_lead__email",
    ]
    ordering_fields = [
        "title",
        "status",
    ...
    def get_queryset(self):
        ...
        if my_engagements and my_engagements.lower() == "true":
            queryset = queryset.filter(
                Q(assigned_auditor=self.request.user)
                | Q(engagement_lead=self.request.user)
            )
            ...
            timeline_events.append(
                {
                    ...
                    "status": "completed"
                    if event.event_type in ["submitted", "closed"]
                    else "in_progress",
                    "icon": event.icon,
                    "color": event.color,
                }
    ...
    def calendar(self, request):
        ...
        if year and month:
            ...
            try:
                ...
                queryset = queryset.filter(
                    planned_start_date__lt=end_date, planned_end_date__gte=start_date
                )
    ...
    def dashboard_metrics(self, request):
        ...
        for criticality_choice in AuditEntity.CRITICALITY_CHOICES:
            entity_coverage["entities_by_criticality"][criticality_choice[0]] = (
                entities.filter(criticality=criticality_choice[0]).count()
            )
        ...
        for entity_type in AuditEntity.ENTITY_TYPES:
            for criticality in AuditEntity.CRITICALITY_CHOICES:
                entities_in_cell = entities.filter(
                    entity_type=entity_type[0], criticality=criticality[0]
                )
        ...
        checklist_stats = {"total_executions": executions.count(), "by_status": {}}
        ...
        checklist_results = {"total_results": item_results.count(), "by_result": {}}
        ...
        # Performance metrics
        completed_engagements = queryset.filter(
            status="closed", actual_end_date__isnull=False
        )
        ...
        if completed_engagements.exists():
            ...
            for engagement in completed_engagements:
                if engagement.actual_start_date and engagement.actual_end_date:
                    duration = (
                        engagement.actual_end_date - engagement.actual_start_date
                    ).days
            ...
            if durations:
                performance_metrics["avg_duration_days"] = sum(durations) / len(
                    durations
                )
                performance_metrics["on_time_completion_rate"] = (
                    on_time_count / len(durations)
                ) * 100

            # Budget variance
            if (
                budget_stats["total_budget_allocated"]
                and budget_stats["total_actual_cost"]
            ):
                variance = (
                    (
                        budget_stats["total_actual_cost"]
                        - budget_stats["total_budget_allocated"]
                    )
                    / budget_stats["total_budget_allocated"]
                    * 100
        ...
        for eng in budget_issues:
            variance = (
                (eng.actual_cost - eng.budget_allocated) / eng.budget_allocated * 100
            )
        ...
        for i in range(11, -1, -1):  # Last 12 months
            ...
            completed = queryset.filter(
                status="closed", closed_at__gte=month_start, closed_at__lte=month_end
            ).count()

            planned = queryset.filter(
                planned_end_date__gte=month_start, planned_end_date__lte=month_end
            ).count()
...
class ChecklistItemViewSet(viewsets.ModelViewSet):
    ...
    queryset = ChecklistItem.objects.all().select_related(
        "checklist", "control", "risk", "policy"
    )
    ...
    def reorder(self, request, pk=None):
        ...
        existing_item = (
            ChecklistItem.objects.filter(checklist=checklist, order=new_order)
            .exclude(id=item.id)
            .first()
        )
...
class ChecklistExecutionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        execution = serializer.save(
            started_by=self.request.user, started_at=timezone.now()
        )
        ...
        ChecklistItemResult.objects.bulk_create(
            [
                ChecklistItemResult(
                    execution=execution, checklist_item=item, result="not_tested"
                )
...
class ChecklistItemResultViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        result = serializer.save(tested_by=self.request.user, tested_at=timezone.now())
