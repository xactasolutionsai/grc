...
class AuditEntity(models.Model):
    ...
    entity_type = models.CharField(max_length=50, choices=ENTITY_TYPES, null=False, blank=False)
    description = models.TextField(blank=True)
    objectives = models.TextField(null=True, blank=True, help_text="Key responsibilities and goals of this entity")
    parent = models.ForeignKey(
        'self', null=True, blank=True, on_delete=models.SET_NULL, related_name='children'
    )
    owner = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='owned_audit_entities', help_text="Entity owner/lead")
    team_member = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='team_member_audit_entities', help_text="Entity team member/assigned person")
    ...
    contact_type = models.CharField(max_length=20, choices=CONTACT_TYPE_CHOICES, default="owner", help_text="Type of contact person")
    key_contact = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='key_contact_audit_entities', help_text="Key contact person for this entity")
    risk_score = models.FloatField(default=0.0, null=True, blank=True)
    inherent_risk_score = models.FloatField(null=True, blank=True, validators=[MinValueValidator(0), MaxValueValidator(10)])
    residual_risk_score = models.FloatField(null=True, blank=True, validators=[MinValueValidator(0), MaxValueValidator(10)])
    control_maturity = models.IntegerField(null=True, blank=True, validators=[MinValueValidator(0), MaxValueValidator(5)])
    ...
    priority = models.CharField(max_length=10, choices=PRIORITY_CHOICES, default="medium", help_text="Priority level for this entity")
    audit_frequency = models.CharField(max_length=20, choices=AUDIT_FREQUENCY_CHOICES, default="annual")
    next_audit_date = models.DateField(null=True, blank=True)
    # Geographical location fields for corporate enterprises
    location = models.CharField(max_length=255, blank=True)  # Keep for backward compatibility
    country = models.CharField(max_length=100, blank=True, help_text="Country where the entity is located")
    region = models.CharField(max_length=100, blank=True, help_text="Region/State/Province")
    city = models.CharField(max_length=100, blank=True, help_text="City where the entity is located")
    ...
    timezone = models.CharField(max_length=50, blank=True, help_text="Timezone (e.g., America/New_York)")
    coordinates = models.JSONField(blank=True, null=True, help_text="Latitude and longitude coordinates")
...
class AuditPlan(models.Model):
    ...
    entity = models.ForeignKey("AuditEntity", on_delete=models.CASCADE, related_name="audit_plans")
    auditable_entities = models.ManyToManyField("AuditEntity", blank=True, related_name="related_audit_plans", help_text="Additional auditable entities associated with this plan")
    title = models.CharField(max_length=255)
    description = models.TextField(blank=True)
    planned_start = models.DateField()
    planned_end = models.DateField()
    actual_start = models.DateField(null=True, blank=True, help_text="Actual start date of the audit")
    actual_end = models.DateField(null=True, blank=True, help_text="Actual end date of the audit")
    lead_auditor = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='led_audit_plans')
    audit_team = models.JSONField(blank=True, null=True, help_text="Audit team members with their roles: [{'user_id': int, 'role': str, 'username': str, 'email': str}]")
    ...
    submitted_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='submitted_audit_plans')
    approved_at = models.DateTimeField(null=True, blank=True)
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='approved_audit_plans')
    rejection_reason = models.TextField(blank=True)
    rejection_date = models.DateTimeField(null=True, blank=True)
    rejected_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='rejected_audit_plans')
...
class AuditPlanApproval(models.Model):
    ...
    audit_plan = models.ForeignKey("AuditPlan", on_delete=models.CASCADE, related_name="approvals")
    approver = models.ForeignKey(User, on_delete=models.CASCADE, related_name="audit_plan_approvals")
...
class AuditEngagement(models.Model):
    ...
    title = models.CharField(max_length=255)
    description = models.TextField(blank=True)
    audit_plan = models.ForeignKey("AuditPlan", on_delete=models.CASCADE, related_name="engagements")
    entity = models.ForeignKey("AuditEntity", on_delete=models.CASCADE, related_name="engagements")
    
    # Engagement details
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="draft")
    priority = models.CharField(max_length=10, choices=PRIORITY_CHOICES, default="medium")
    audit_type = models.CharField(max_length=30, choices=AUDIT_TYPE_CHOICES, default="internal", null=True, blank=True)
    assigned_auditor = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='assigned_engagements')
    engagement_lead = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='led_engagements')
    ...
    results_submitted_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='submitted_engagements')
    closed_at = models.DateTimeField(null=True, blank=True)
    closed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='closed_engagements')
    closure_notes = models.TextField(blank=True)
    
    # Resources and budget
    estimated_hours = models.IntegerField(null=True, blank=True, validators=[MinValueValidator(0)])
    actual_hours = models.IntegerField(null=True, blank=True, validators=[MinValueValidator(0)])
...
class EngagementTimelineEvent(models.Model):
    ...
    engagement = models.ForeignKey("AuditEngagement", on_delete=models.CASCADE, related_name="timeline_events")
    ...
    progress_percentage = models.IntegerField(null=True, blank=True, validators=[MinValueValidator(0), MaxValueValidator(100)])
...
class EntityProcessActivity(models.Model):
    ...
    audit_entity = models.ForeignKey("AuditEntity", on_delete=models.CASCADE, related_name="process_activities")
    identifier = models.CharField(max_length=50, blank=True)
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True)
    depends_on = models.ForeignKey("self", null=True, blank=True, related_name="dependents", on_delete=models.SET_NULL)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["audit_entity", "identifier"], name="uniq_entity_process_identifier")
        ]
    ...
    def _next_sequence_for_entity(self, code: str) -> int:
        ...
        existing = self.audit_entity.process_activities.filter(identifier__startswith=f"{code}-").values_list("identifier", flat=True)
        max_n = 0
...
class Checklist(NameDescriptionMixin, FolderMixin, PublishInRootFolderMixin):
    ...
    is_published = models.BooleanField(default=True, help_text="Whether the checklist is published")
...
class ChecklistItem(models.Model):
    ...
    title = models.CharField(
        max_length=500, 
        help_text="Title of the audit test/question"
    )
    description = models.TextField(
        blank=True, 
...
class ChecklistExecution(models.Model):
    ...
    # Audit trail
    started_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='started_executions')
    started_at = models.DateTimeField(null=True, blank=True)
    completed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='completed_executions')
