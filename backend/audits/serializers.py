...
from core.models import ComplianceAssessment, RiskScenario, AppliedControl, Finding, Policy

class EntityProcessActivitySerializer(serializers.ModelSerializer):
    id = serializers.IntegerField(read_only=True)
    depends_on = serializers.SerializerMethodField()
    depends_on_id = serializers.IntegerField(write_only=True, required=False, allow_null=True)
    ...
    def create(self, validated_data):
        ...
        if depends_on_id:
            try:
                validated_data['depends_on'] = EntityProcessActivity.objects.get(id=depends_on_id)
...
class AuditEntitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'created_at', 'updated_at', 'parent_name', 'owner_username', 'owner_display', 'team_member_username', 'team_member_display', 'key_contact_username', 'key_contact_display', 'children', 'full_location', 'coordinates', 'org_structure_url', 'processes']
    ...
    def get_processes(self, obj):
        ...
        if qs is None:
            return []
        return EntityProcessActivitySerializer(qs.all().order_by('name'), many=True).data
    ...
    def validate_inherent_risk_score(self, value):
        ...
        if value is not None and (value < 0 or value > 10):
            raise serializers.ValidationError("Inherent risk score must be between 0 and 10")
        return value
    
    def validate_residual_risk_score(self, value):
        """Validate residual risk score is between 0 and 10"""
        if value is not None and (value < 0 or value > 10):
            raise serializers.ValidationError("Residual risk score must be between 0 and 10")
        return value
    
    def validate_control_maturity(self, value):
        """Validate control maturity is between 0 and 5"""
        if value is not None and (value < 0 or value > 5):
            raise serializers.ValidationError("Control maturity must be between 0 and 5")
        return value
    ...
    def validate_entity_type(self, value):
        ...
        if value not in valid_choices:
            raise serializers.ValidationError(f"Entity type must be one of: {', '.join(valid_choices)}")
        return value
    ...
    def validate_contact_type(self, value):
        ...
        if value not in valid_choices:
            raise serializers.ValidationError(f"Contact type must be one of: {', '.join(valid_choices)}")
        return value
    
    def validate_parent(self, value):
        """Validate parent is not self (prevent circular references)"""
        if value and hasattr(self, 'instance') and self.instance and value.id == self.instance.id:
    ...
    def validate_coordinates(self, value):
        ...
        if value is not None:
            ...
            if 'latitude' not in value or 'longitude' not in value:
                raise serializers.ValidationError("Coordinates must contain 'latitude' and 'longitude' keys")
            try:
...
class AuditPlanSerializer(serializers.ModelSerializer):
    ...
    auditable_entities = serializers.PrimaryKeyRelatedField(
        many=True, 
        queryset=AuditEntity.objects.all(), 
        required=False,
        allow_null=True
    )
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'created_at', 'updated_at', 'entity_name', 'entity_type', 
            'auditable_entities_display', 'audit_team_display',
            'lead_auditor_display', 'submitted_by_display', 'approved_by_display', 
            'rejected_by_display', 'can_be_approved', 'can_be_rejected',
            'submitted_for_approval_at', 'submitted_by', 'approved_at', 'approved_by',
            'rejection_date', 'rejected_by'
        ]
        ...
        if value:
            ...
            if planned_start:
                ...
                if isinstance(planned_start, str):
                    planned_start = datetime.strptime(planned_start, '%Y-%m-%d').date()
                if value < planned_start:
                    raise serializers.ValidationError("Actual start date cannot be before planned start date")
        return value
    ...
    def validate_audit_team(self, value):
        ...
        if value:
            ...
            lead_auditor_count = sum(1 for member in value if member.get('role') == 'lead_auditor')
            if lead_auditor_count == 0:
                raise serializers.ValidationError("At least one Lead Auditor must be assigned to the audit team")
            
            # Validate structure of each team member
            valid_roles = ['lead_auditor', 'senior_auditor', 'junior_auditor', 'auditor']
            for member in value:
                if not isinstance(member, dict):
                    raise serializers.ValidationError("Each team member must be an object")
                if 'user_id' not in member or 'role' not in member:
                    raise serializers.ValidationError("Each team member must have 'user_id' and 'role'")
                if member['role'] not in valid_roles:
                    raise serializers.ValidationError(f"Invalid role '{member['role']}'. Must be one of: {', '.join(valid_roles)}")
        return value
    ...
    def update(self, instance, validated_data):
        ...
        try:
            if auditable_entities is not None and hasattr(audit_plan, 'auditable_entities'):
                audit_plan.auditable_entities.set(auditable_entities)
        except Exception as e:
            # Field doesn't exist yet, migration not applied
            pass
...
class AuditPlanApprovalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'created_at', 'updated_at', 'audit_plan_title', 'approver_display', 'approved_at']
    ...
    def validate_status(self, value):
        ...
        valid_choices = [choice[0] for choice in self.Meta.model.APPROVAL_STATUS_CHOICES]
        if value not in valid_choices:
...
class AuditEngagementSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'created_at', 'updated_at', 'audit_plan_title', 'entity_name', 'entity_type',
            'assigned_auditor_display', 'engagement_lead_display', 'results_submitted_by_display',
            'closed_by_display', 'created_by_display', 'can_be_started', 'can_submit_results',
            'can_be_closed', 'duration_days', 'actual_duration_days', 'is_overdue',
    ...
    def get_results_submitted_by_display(self, obj):
        ...
        try:
            return obj.results_submitted_by.username if obj.results_submitted_by else None
        except AttributeError:
            return None
    ...
    def validate_planned_end_date(self, value):
        ...
        if planned_start_date and value:
            ...
            if isinstance(planned_start_date, str):
                planned_start_date = datetime.strptime(planned_start_date, '%Y-%m-%d').date()
    ...
    def validate_priority(self, value):
        ...
        if value not in valid_choices:
            raise serializers.ValidationError(f"Priority must be one of: {', '.join(valid_choices)}")
        return value
    ...
    def validate_audit_type(self, value):
        ...
        if value not in valid_choices:
            raise serializers.ValidationError(f"Audit type must be one of: {', '.join(valid_choices)}")
        return value
    ...
    def validate_actual_start_date(self, value):
        ...
        if value:
            planned_start_date = self.initial_data.get('planned_start_date')
            if self.instance:
                planned_start_date = planned_start_date or self.instance.planned_start_date
...
class AuditEngagementActionSerializer(serializers.Serializer):
    ...
    def validate_action(self, value):
        ...
        if value not in ['start', 'submit_results', 'close']:
            raise serializers.ValidationError("Action must be 'start', 'submit_results', or 'close'")
        return value
...
class ChecklistItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            ...
            'control_name', 'control_display',
            'risk_name', 'risk_display',
            'policy_name', 'policy_display'
        ]
...
class ChecklistSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'created_at', 'updated_at', 
            'folder_name', 'created_by_display', 
            'items', 'item_count'
        ]
...
class ChecklistItemResultSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'execution', 'checklist_item', 'checklist_item_title', 
            'checklist_item_description', 'checklist_item_order',
            'result', 'comments', 'finding_summary',
            'tested_by', 'tested_by_display', 'tested_at',
            'control_name', 'risk_name', 'policy_name',
    ...
    def get_tested_by_display(self, obj):
        ...
        if obj.tested_by:
            return f"{obj.tested_by.first_name} {obj.tested_by.last_name}".strip() or obj.tested_by.username
        return None
...
class ChecklistExecutionSerializer(serializers.ModelSerializer):
    ...
    engagement_title = serializers.CharField(source='audit_engagement.title', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'checklist', 'checklist_name', 'audit_engagement', 'engagement_title',
            'status', 'started_by', 'started_by_display', 'started_at',
            'completed_by', 'completed_by_display', 'completed_at',
            'total_items', 'completed_items', 'progress_percentage',
            'notes', 'item_results', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'total_items', 'completed_items', 'created_at', 'updated_at']
    
    def get_started_by_display(self, obj):
        """Get started by display name"""
        if obj.started_by:
            return f"{obj.started_by.first_name} {obj.started_by.last_name}".strip() or obj.started_by.username
        return None
    ...
    def get_item_results(self, obj):
        ...
        results = obj.item_results.select_related('checklist_item', 'tested_by').order_by('checklist_item__order')
