...
class WorkpaperAPITestCase(TestCase):
    ...
    def test_list_workpapers(self):
        ...
        Workpaper.objects.create(
            title="Workpaper 1", workpaper_type="pdf", uploaded_by=self.user
        )
        Workpaper.objects.create(
            title="Workpaper 2", workpaper_type="excel", uploaded_by=self.user
        )
    ...
    def test_update_workpaper(self):
        ...
        response = self.client.put(
            f"/api/workpapers/workpapers/{workpaper.id}/", data, format="json"
        )
    ...
    def test_submit_for_review(self):
        ...
        response = self.client.post(
            f"/api/workpapers/workpapers/{workpaper.id}/submit_for_review/"
        )
    ...
    def test_approve_workpaper(self):
        ...
        response = self.client.post(
            f"/api/workpapers/workpapers/{workpaper.id}/approve/", data, format="json"
        )
    ...
    def test_reject_workpaper(self):
        ...
        data = {"action": "reject", "reason": "Needs more detail"}

        response = self.client.post(
            f"/api/workpapers/workpapers/{workpaper.id}/reject/", data, format="json"
        )
    ...
    def test_filter_by_type(self):
        ...
        Workpaper.objects.create(
            title="PDF WP", workpaper_type="pdf", uploaded_by=self.user
        )
        Workpaper.objects.create(
            title="Excel WP", workpaper_type="excel", uploaded_by=self.user
        )
