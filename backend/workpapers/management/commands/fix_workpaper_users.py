        ...
        if count > 0:
            ...
            if dry_run:
                ...
                for wp in workpapers_without_owner:
                    self.stdout.write(
                        f"  - Would set uploaded_by for: {wp.title} (ID: {wp.id})"
                    )
            else:
                updated = workpapers_without_owner.update(uploaded_by=default_user)
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Updated {updated} workpaper(s) with uploaded_by"
                    )
                )
        else:
            self.stdout.write(
                self.style.SUCCESS("✓ All workpapers have uploaded_by set")
            )

        # Fix approval history without action_by
        approvals_without_user = WorkpaperApproval.objects.filter(
        ...
        if count > 0:
            ...
            if dry_run:
                ...
                for approval in approvals_without_user:
                    self.stdout.write(
                        f"  - Would set action_by for: {approval.workpaper.title} - {approval.action} (ID: {approval.id})"
                    )
            else:
                updated = approvals_without_user.update(action_by=default_user)
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Updated {updated} approval record(s) with action_by"
                    )
                )
        else:
            self.stdout.write(
                self.style.SUCCESS("✓ All approval records have action_by set")
            )

        # Fix workpapers with 'reviewed' status but no reviewer
        reviewed_without_reviewer = Workpaper.objects.filter(
            status="reviewed", reviewer__isnull=True
        )
        ...
        if count > 0:
            ...
            if dry_run:
                ...
                for wp in reviewed_without_reviewer:
                    self.stdout.write(
                        f"  - Would set reviewer for: {wp.title} (ID: {wp.id})"
                    )
            else:
                updated = reviewed_without_reviewer.update(reviewer=default_user)
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Updated {updated} workpaper(s) with reviewer"
                    )
                )
        else:
            self.stdout.write(
                self.style.SUCCESS("✓ All reviewed workpapers have reviewer set")
            )

        # Fix workpapers with 'approved' status but no approver
        approved_without_approver = Workpaper.objects.filter(
            status="approved", approver__isnull=True
        )
        ...
        if count > 0:
            ...
            if dry_run:
                ...
                for wp in approved_without_approver:
                    self.stdout.write(
                        f"  - Would set approver for: {wp.title} (ID: {wp.id})"
                    )
            else:
                updated = approved_without_approver.update(approver=default_user)
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Updated {updated} workpaper(s) with approver"
                    )
                )
        else:
            self.stdout.write(
                self.style.SUCCESS("✓ All approved workpapers have approver set")
            )
